...
socket.on('chat-message', (data) => {
  addMessageToUI(data);  // Display message on both sender's and receiver's side
});

// Display messages on the UI
function addMessageToUI(data) {
  clearFeedback();
  const isOwnMessage = data.name === nameInput.value;  // Check if the message is from the sender
  const element = `
